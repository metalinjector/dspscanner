...
# Пояснение к безопасному удалению. Вынесено в константу: текст длинный и
# описывает границы применимости, о которых пользователь обязан знать до
# того, как понадеется на кнопку.
_SECURE_ERASE_HELP = """
<style>
  h3 { margin: 14px 0 4px 0; }
  p, li { line-height: 140%; }
  ol, ul { margin: 2px 0 0 0; -qt-list-indent: 1; }
  li { margin-bottom: 3px; }
</style>

<h3>Что делает программа</h3>
<ol>
  <li>Перезаписывает содержимое файла — число проходов задаётся
      в «Настройках».</li>
  <li>Обнуляет размер: сама длина файла тоже кое-что о нём говорит.</li>
  <li>Несколько раз переименовывает файл случайным именем — иначе имя
      вроде <i>увольнение_иванова.docx</i> останется в записи каталога.</li>
  <li>Удаляет файл и сбрасывает запись каталога на диск.</li>
</ol>

<h3>Где это надёжно</h3>
<p>На жёстких дисках (HDD), флешках и картах памяти перезапись
действительно затирает прежнее содержимое. По современным стандартам
(NIST SP 800-88) достаточно <b>одного прохода</b>; большее число —
дань устаревшим правилам вроде DoD 5220.22-M и заметно медленнее.</p>

<h3>Где перезапись не даёт гарантии</h3>
<p>На <b>SSD и NVMe</b> контроллер сам решает, в какую физическую ячейку
писать (wear leveling), и держит резервную область (over-provisioning),
недоступную программам. Новая запись почти всегда ложится в другое место,
а старая копия может уцелеть в резервных ячейках. Число проходов этого
не меняет — лишние проходы только изнашивают диск.</p>
<p>То же касается:</p>
<ul>
  <li>файловых систем с журналом и снимками — APFS, Btrfs, ZFS,
      NTFS с теневыми копиями;</li>
  <li>сетевых и облачных дисков, а также резервных копий;</li>
  <li>временных файлов, созданных Word, LibreOffice или самой ОС.</li>
</ul>

<h3>Если данные действительно секретные</h3>
<ul>
  <li><b>Полное шифрование диска</b> — BitLocker, FileVault, LUKS,
      включённое <b>заранее</b>: тогда на диске нет открытых данных,
      а уничтожение сводится к потере ключа.</li>
  <li><b>Очистка всего носителя</b> встроенными командами диска:
      ATA Secure Erase, NVMe Sanitize или криптостирание из фирменной
      утилиты производителя.</li>
  <li><b>Физическое уничтожение</b> носителя, если требования
      максимальные.</li>
</ul>

<p style="margin-top: 14px;">Эта кнопка защищает от восстановления
обычными программами вроде Recuva или R-Studio. Она не рассчитана на
противодействие лаборатории, читающей чипы памяти напрямую.</p>
"""
...
class MainWindow(QMainWindow):
    ...
    def _help_button(self, title: str, text: str) -> QPushButton:
        ...
        button.clicked.connect(lambda: self._show_help(title, text))
        return button
    ...
    def _build_operations_box(self) -> QGroupBox:
        ...
        warning = QLabel(
            "Затираются содержимое, имя и размер. На SSD/NVMe перезапись "
            "не даёт гарантии — подробности по «?»."
        )
        warning.setWordWrap(True)
        warning.setStyleSheet("color: #9AA3B2;")
        row.addWidget(warning, 1)
        # Ограничения перезаписи важнее прятать нельзя: пользователь принимает
        # решение об уничтожении улик, полагаясь на эту кнопку.
        row.addWidget(self._help_button("Что делает безопасное удаление", _SECURE_ERASE_HELP))
        return box
    ...
    def _secure_delete_checked(self) -> None:
        ...
        confirm = QMessageBox.question(
            ...
            "Содержимое, имя и размер будут затёрты, файлы удалены. "
            "Действие необратимо.\n\n"
            "На SSD/NVMe перезапись не гарантирует уничтожения: часть данных "
            "может остаться в резервных ячейках диска.",
        )
    ...
    def _secure_move_checked(self) -> None:
        ...
        confirm = QMessageBox.question(
            ...
            "Сначала будет создана и проверена копия, затем оригинал будет "
            "затёрт и удалён с прежнего места.\n\n"
            "На SSD/NVMe перезапись не гарантирует уничтожения оригинала: "
            "часть данных может остаться в резервных ячейках диска.",
        )
        if confirm != QMessageBox.Yes:
            return
    ...
    def _show_about(self) -> None:
        QMessageBox.about(
            ...
            "<li>Автосохранение параметров в переносимый JSON (DSPScanner-Config)</li>"
            "<li>Копирование и безопасное удаление файлов (с оговорками для SSD)</li>"
            "</ul>",
        )
