...
_SECURE_ERASE_HELP = (
    ...
    "1. Перезаписывает содержимое файла (число проходов — в «Настройках»).\n"
    "2. Обнуляет размер: сама длина файла тоже кое-что говорит о нём.\n"
    "3. Несколько раз переименовывает файл случайным именем — иначе имя вроде\n"
    "   «увольнение_иванова.docx» останется в записи каталога.\n"
    "4. Удаляет файл и сбрасывает запись каталога на диск.\n\n"
    "Где это надёжно\n"
    "На жёстких дисках (HDD), флешках и картах памяти перезапись действительно\n"
    "затирает прежнее содержимое. Одного прохода по современным стандартам\n"
    "(NIST SP 800-88) уже достаточно; больше проходов — дань старым правилам\n"
    "вроде DoD 5220.22-M и заметно медленнее.\n\n"
    "Где перезапись НЕ даёт гарантии\n"
    "На SSD и NVMe контроллер сам решает, в какую физическую ячейку писать\n"
    "(wear leveling), и держит резервную область (over-provisioning), недоступную\n"
    "программам. Новая запись почти всегда ложится в другое место, а старая копия\n"
    "может остаться в резервных ячейках. Никакое число проходов этого не меняет —\n"
    "наоборот, лишние проходы только изнашивают диск.\n\n"
    "То же касается: файловых систем с журналом и снапшотами (APFS, Btrfs, ZFS,\n"
    "NTFS с теневыми копиями), сетевых и облачных дисков, резервных копий, а\n"
    "также временных файлов, которые могли создать Word, LibreOffice или сама ОС.\n\n"
    "Что делать, если данные действительно секретные\n"
    "• Полное шифрование диска (BitLocker, FileVault, LUKS), включённое ЗАРАНЕЕ:\n"
    "  тогда на диске нет открытых данных, а уничтожение сводится к потере ключа.\n"
    "• Для очистки всего носителя — встроенные команды диска: ATA Secure Erase,\n"
    "  NVMe Sanitize или криптостирание из фирменной утилиты производителя.\n"
    "• Физическое уничтожение носителя, если требования максимальные.\n\n"
    "Эта кнопка защищает от восстановления обычными программами\n"
    "(Recuva, R-Studio и подобными). Она не рассчитана на противодействие\n"
    "лаборатории, снимающей данные с чипов памяти напрямую."
)
