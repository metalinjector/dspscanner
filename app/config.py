...
class ScanSettings:
    ...
    # Предварительный обход дерева ради общего числа файлов до начала чтения.
    # Даёт прогресс-бару неподвижную точку отсчёта: без него знаменатель растёт
    # по мере поиска файлов и заполнение полосы «едет назад». Стоимость —
    # задержка перед стартом, поэтому на очень медленных сетевых дисках
    # подсчёт можно отключить и вернуться к потоковому режиму.
    precount_files: bool = True
